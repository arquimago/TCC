...
__global__ void SPSOMultiEnxame(parametros* param, exemplo* exemplos, int quant_exemp, atributo* atrib, int quant_atrib, FILE* output, particula* enxames, regra* posicoesFinais, curandState* globalState) {
	...
	//CALCULA POSIÇÃO DO VETOR

	int div, resto, valor_vect, quant_vect;
	div = quant_exemp / (*param).quant_enxames;
	resto = quant_exemp % (*param).quant_enxames;
	valor_vect = id >= (div*(*param).quant_enxames) ? ((id - resto) / ((*param).quant_enxames)) : (id / (*param).quant_enxames);
	quant_vect = div*(*param).quant_particulas;
	quant_vect += id >= (div*(*param).quant_enxames) ? resto : 0;

	//CARREGA PARTÍCULA, CALCULA FUNÇÕES OBJETIVO E PREENCHE ENXAME DO RESPECTIVO BLOCO

	particula p = enxames[id];
	device_preencheMatrizCont(&(p.posicao), &exemplos[valor_vect], quant_vect, quant_atrib);
	device_calculaFuncoesObj(&(p.posicao), quant_atrib, atrib, quant_vect);
	...
	for (i = 0; i < (*param).bl_interacoes; i++) {
		...
		device_preencheMatrizCont(&(enxame_shared[threadIdx.x].posicao), &exemplos[valor_vect], quant_vect, quant_atrib);
		device_calculaFuncoesObj(&(enxame_shared[threadIdx.x].posicao), quant_atrib, atrib, quant_vect);
